...
#'
#' @description
#' This function computes posterior chain analysis with Bayesian Statistics. Posterior chains need to be contained in a list generated by the `bunny` function. It evaluates model means, treatments, covariates, comparisons, and variance components using Bayesian inference methods.
#'
#' @details
...
#' @param HPD A scalar value between 0 and 1 specifying the desired probability to compute the highest posterior density (HPD) interval. Default value is 0.95. The HPD interval is computed for model mean, treatments, covariates, comparisons, and variance components.
#' @param P0 A logical (default is `TRUE`). If `TRUE`, computes the probability that the posterior distribution is greater than zero if the median is positive, or less than zero if the median is negative. This applies to treatments, comparisons, and covariates. For comparisons between levels of treatments expressed as ratios, it computes probabilities of being greater than or less than 1.
#' @param K A logical(default is `FALSE`). If `TRUE`, computes a "guaranteed" value of the posterior distribution with a specified probability (`probK`). This value is only displayed if its sign matches the sign of the median of the posterior distribution. Applies to treatments, comparisons, covariates, and variance components.
...
#'
#' @return Returns a list with computed inferences for each trait, including model means, treatments, comparisons, covariates, variance components, and probabilities associated with specified thresholds.
...
Bayes <- function(params, bunny, HPD=0.95, P0=TRUE,
  ...
  #3) If plot=T make plots of the comparisons
  ...
  if (plot && length(bunny[[trait]][["Compare"]]) > 0) {
    for (trait in names(bunny)) {
      ...
      cPlot <- ggplot(plot_data_long, aes(x = value, y = comparison)) +  # Ensure 'condition' is defined as per your logic
        ...
        theme_classic() +
        labs(
          y = "Comparisons Between Levels of Treatments",
      ...
      file_name <- paste0("Comparisons_posterior_plot_", trait, ".tiff")
