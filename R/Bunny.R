...
#'
#' @description
#' This function fits a mixed-effects model to the provided dataset based on specifications in a parameter file. It handles fixed effects, random effects, and interactions of order 2 between fixed effects, performs convergence diagnosis, computes least squares means (LSMeans), and calculates comparisons between treatment levels as defined. This function makes extensive use of MCMCglmm function \insertCite{Hadfield2010mcmc}{RabbitR}.
#'
#' @param params A parameter file generated by either `iCreateParam` or `CreateParam`.
#' @param Chain Logical (default=FALSE).  logical value (default=`FALSE`). If set to `TRUE`, the function writes down an excel file with all samples from the posterior distributions of the estimated parameters for each trait for post-hoc analysis.
#' @return A comprehensive list detailing the outcomes of the mixed-effects model analysis. This includes posterior distribution samples for estimated parameters: model mean, residual variance, treatment means, comparisons between treatment levels, covariates and variance components of random effects.
#'
#'
#' @import MCMCglmm
#' @import coda
...
#' \dontrun{
#' # Example
#' results <- Bunny(params = my_param_file, Chain = TRUE)
#' # To run the analysis without storing the chains
#' results <- Bunny(params = my_param_file)
#'}
#'
#' @references
#' \insertAllCited{}
...
Bunny <- function(params, Chain=FALSE) {
  ...
  if (length(params$hInter) > 0) {
    interaction <- c()
    for (i in 1:nrow(params$hInter)) {
      interaction_pair <- params$hInter[i, ]
      # Check if both elements of the interaction are declared as treatments, noise or covariates
      if (!all(interaction_pair %in% c(params$hTreatment, params$hNoise, params$hCov))) {
        stop(paste("Error: Interaction components", toString(interaction_pair), "are not all present in hTreatment, hNoise, or hCov."))
      }
      # Create interaction term
      interaction <- c(interaction, paste(interaction_pair[1], interaction_pair[2], sep = "*"))
    }
  ...
  for (n in seq_along(params$hTrait)) {
    ...
    # Select columns from your data that match with hTreatment, hNoise or hCov (and so, interactions)
    ...
    computeLSmeans<-function(params, data, model) {
      ...
      # Generate levels for noise if they exist (make sue they are factors)
      if (length(params$hNoise) > 0) {
      ...
      for (varName in names(categoricalAndCovariateVarsLevels)) {
        if (varName %in% c(params$hTreatment, params$hNoise)) {
          #Convert to factor if not already
          refGrid[[varName]] <- factor(refGrid[[varName]])
        }
      }

      # Adding interactions in refGrid (Optional)
      if (length(params$hInter)>0) {
        for (i in 1:nrow(params$hInter)) {
          interaction_vars <- params$hInter[i, ]
          refGrid[[paste(interaction_vars, collapse = ":")]] <- interaction(
            refGrid[[interaction_vars[1]]], refGrid[[interaction_vars[2]]], drop = TRUE
          )
        }
      }
      ...
      all_factors <- setdiff(names(refGrid), params$hCov)  # Exclude covariates from LSMeans computation
    ...
    # If any of hTreatment or hNoise have a length greater than 0, compute LSMeans
    if (length(params$hTreatment) > 0 || length(params$hNoise) > 0) {

      # Compute LSmeans
      cat("\nComputing Means... \n")
      LSmeans <- computeLSmeans(params, data, model)

      # Display a message that LSMeans have been computed
      cat("\nMeans computed \n")
      cat("---------------------------------------------------\n")
    } else {
    ...
    if (length(params$hNoise) > 0) {
      noiseMeans <- LSmeans[names(LSmeans) %in% params$hNoise] #OJO!! Include nInter here if ShowInter="N"
    }

    # Assign Lsmeans of interactions to Treatment or Noise (optional)
    if (length(params$hInter)>0 && length(params$ShowInter)>0) {

      # Check if the number of interaction classifications matches the number of interactions
      if (nrow(params$hInter) != length(params$ShowInter)) {
        stop("Error: The number of interactions in 'ShowInter' and 'hInter' does not match .")
      }

      # Include interactions classified as treatments or noise
      for (i in 1:nrow(params$hInter)) {
        interaction_name <- paste(params$hInter[i, 1], params$hInter[i, 2], sep = ":")
        interaction_show <- params$ShowInter[i]

        # Check if the interaction is classified as a treatment or noise
        if (interaction_show == "T" && interaction_name %in% names(LSmeans)) {
          # Add interaction to treatments
          treatMeans[[interaction_name]] <- LSmeans[[interaction_name]]
        } else if (interaction_show == "N" && interaction_name %in% names(LSmeans)) {
          # Add interaction to noise
          noiseMeans[[interaction_name]] <- LSmeans[[interaction_name]]
        }
      }
    }
    ...
    if (length(treatMeans) > 0) {
      ...
      for (treatment in names(treatMeans)) {
        ...
        # Get levels directly from treatMeans
        levels <- names(treatMeans[[treatment]])

        # Compute pairwise combinations of levels
        level_combinations <- combn(levels, 2, simplify = FALSE)

        # Iterate over each combination to compute comparisons
        for (combination in level_combinations) {
          ...
          # Compute comparisons based on user's choice
          ...
          # Construct the name for this particular comparison
          contrast_name <- paste(level1, separator, level2, sep="")

          # Store comparisons in the appropriate treatment list within contrasts_list
      ...
      cat("\nNo Treatment effects specified. No comparisons to summarize.\n")
      contrasts_list <- list()
    }
    ...
    if ((Chain) == TRUE) {
      ...
      if (length(params$hTreatment) > 0) {
        ...
        writeData(OUT, sheet = "Comparisons", x = contrasts_df, colNames = TRUE, rowNames = FALSE)
      }
      ...
      cat("\n")
      cat("Posterior samples for trait ", trait, " saved \n")
      cat("---------------------------------------------------\n")
    }
